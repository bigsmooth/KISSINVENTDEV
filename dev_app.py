...
if role == "Hub Manager" and st.experimental_get_query_params().get("dashboard") == ["true"]:
    ...
    # 1. Inventory Summary
    inv_data = query("SELECT sku, quantity FROM inventory WHERE hub=?", (hub,))
    df_inv = pd.DataFrame(inv_data, columns=["SKU", "Qty"])
    total_units = df_inv["Qty"].sum()
    total_skus = df_inv.shape[0]
    low_stock_count = df_inv[df_inv["Qty"] < 10].shape[0]
    ...
    # 2. Recent Inventory Logs
    logs = query("SELECT timestamp, sku, action, qty, comment FROM logs WHERE hub=? ORDER BY timestamp DESC LIMIT 3", (hub,))
    if logs:
        st.subheader("🕒 Recent Inventory Actions")
        st.table(pd.DataFrame(logs, columns=["Time", "SKU", "Action", "Qty", "Comment"]))
    
    # 3. Top 5 Moved SKUs
    sku_logs = query("SELECT sku, COUNT(*) FROM logs WHERE hub=? GROUP BY sku ORDER BY COUNT(*) DESC LIMIT 5", (hub,))
    if sku_logs:
        st.subheader("📈 Top 5 Moving SKUs")
        top_df = pd.DataFrame(sku_logs, columns=["SKU", "Actions"])
        st.bar_chart(top_df.set_index("SKU"))
    
    # 4. Pending Shipments
    incoming = query("SELECT COUNT(*) FROM shipments WHERE hub=? AND status='Pending'", (hub,))
    pending_count = incoming[0][0] if incoming else 0
    st.markdown("---")
    st.subheader("📦 Pending Shipments")
    st.write(f"You have **{pending_count}** unconfirmed shipment(s).")

    # 5. Last Inventory Count
    last_count = query("SELECT confirmed_at FROM count_confirmations WHERE hub=? ORDER BY confirmed_at DESC LIMIT 1", (hub,))
    if last_count:
        st.write(f"✅ Last count confirmed on: **{last_count[0][0]}**")
    else:
        st.write("❌ No inventory count confirmed yet.")
...
# --- Recycle Bin ---
if menu == "Recycle Bin" and role == "Admin":
    st.title("🗑️ Recycle Bin")

    tab1, tab2 = st.tabs(["🧍 Deleted Users", "📦 Deleted SKUs"])

    with tab1:
        deleted_users = query("SELECT username, role, hub FROM users WHERE deleted=1")
        if deleted_users:
            for user in deleted_users:
                st.write(f"🧍 **{user[0]}** | Role: {user[1]} | Hub: {user[2]}")
    ...
    with tab2:
        deleted_skus = query("SELECT sku, product_name FROM sku_info WHERE deleted=1")
        if deleted_skus:
            for sku in deleted_skus:
                st.write(f"📦 **{sku[0]}** - {sku[1]}")
...
# --- Admin Dashboard ---
if menu == "📊 Admin Dashboard" and role == "Admin":
    st.title("📊 Admin Inventory Dashboard")

    # Inventory Stats
    total_units = query("SELECT SUM(quantity) FROM inventory")[0][0] or 0
    total_skus = query("SELECT COUNT(DISTINCT sku) FROM sku_info")[0][0] or 0
    low_stock = query("SELECT COUNT(*) FROM inventory WHERE quantity < 10")[0][0] or 0
    hubs = query("SELECT DISTINCT hub FROM inventory")

    col1, col2, col3 = st.columns(3)
    col1.metric("🧾 Total SKUs", total_skus)
    col2.metric("📦 Total Units", total_units)
    col3.metric("🟥 Low Stock SKUs", low_stock)

    st.markdown("---")
    
    # Top Movers Across System
    top_logs = query("SELECT sku, COUNT(*) FROM logs GROUP BY sku ORDER BY COUNT(*) DESC LIMIT 10")
    if top_logs:
        st.subheader("📈 Top 10 Moved SKUs (IN/OUT)")
        top_df = pd.DataFrame(top_logs, columns=["SKU", "Actions"])
        st.bar_chart(top_df.set_index("SKU"))

    st.markdown("---")

    # Last Count by Hub
    st.subheader("📅 Last Inventory Count by Hub")
    counts = query("SELECT hub, MAX(confirmed_at) FROM count_confirmations GROUP BY hub")
    if counts:
        df = pd.DataFrame(counts, columns=["Hub", "Last Count"])
        st.table(df)

    st.markdown("---")

    # System-Wide Inventory Snapshot
...
# --- Undo Last Action ---
if menu == "Undo Last Action" and role in ["Admin", "Hub Manager", "Retail"]:
    st.title("🔁 Undo Last Inventory Action")

    # Get last log entry by this user and hub
    last_log = query("SELECT rowid, * FROM logs WHERE user=? AND hub=? ORDER BY timestamp DESC LIMIT 1", (username, hub))
    if last_log:
        log_id, ts, usr, sku, hub_name, action, qty, comment = last_log[0]
        st.info(f"Last Action: **{action} {qty}x {sku}** at {ts}")
        if st.button("Undo This Action", key="undo_action_btn"):
            # Reverse the action
            reverse = "OUT" if action == "IN" else "IN"
            new_comment = f"UNDO of #{log_id}: {action} {qty}"
            # Update inventory
            delta = qty if reverse == "IN" else -qty
            query("UPDATE inventory SET quantity = quantity + ? WHERE sku=? AND hub=?", (delta, sku, hub))
            # Log the undo
            query("INSERT INTO logs VALUES (?, ?, ?, ?, ?, ?, ?)",
                (datetime.now(), username, sku, hub, reverse, qty, new_comment))
            st.success(f"✅ Reversed: {action} {qty} → {reverse} {qty}")
    else:
        st.warning("No recent inventory actions found.")
...
if menu == "Shipments":
 ...
 # --- Supplier Dashboard ---
if menu == "📊 Supplier Dashboard" and role == "Supplier":
    st.title("📊 Supplier Shipment Dashboard")

    # Count of shipments
    shipment_count = query("SELECT COUNT(*) FROM shipments WHERE user=?", (username,))[0][0]
    total_units = query("SELECT SUM(qty) FROM shipment_skus WHERE shipment_id IN (SELECT id FROM shipments WHERE user=?)", (username,))[0][0] or 0
    pending = query("SELECT COUNT(*) FROM shipments WHERE user=? AND status='Pending'", (username,))[0][0]

    col1, col2, col3 = st.columns(3)
    col1.metric("📦 Total Shipments", shipment_count)
    col2.metric("📦 Units Shipped", total_units)
    ...
    shipments = query("SELECT id, tracking, carrier, hub, ship_date, status FROM shipments WHERE user=? ORDER BY ship_date DESC LIMIT 10", (username,))
    df = pd.DataFrame(shipments, columns=["ID", "Tracking", "Carrier", "Hub", "Date", "Status"])
    st.dataframe(df, use_container_width=True)
